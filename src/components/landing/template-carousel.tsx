...
/**
 * Ids are tied to `PdfLayoutId` so retiring a layout is a type error here
 * rather than a 404 on the generated screenshot at runtime.
 */
type TemplateMeta<Id extends string> = {
  id: Id;
  label: string;
  /** Screenshot generated by `pnpm screenshots` (public/templates/<id>.webp). */
  src: string;
};

const TEMPLATES_TOP: TemplateMeta<PdfLayoutId>[] = (
  [
  ...
  ] satisfies ReadonlyArray<{ id: PdfLayoutId; label: string }>
).map((t) => ({ ...t, src: `/templates/${t.id}.webp` }));

const TEMPLATES_BOTTOM: TemplateMeta<`${PdfLayoutId}-alt`>[] = (
  [
  ...
  ] satisfies ReadonlyArray<{ id: `${PdfLayoutId}-alt`; label: string }>
).map((t) => ({ ...t, src: `/templates/${t.id}.webp` }));

function TemplateCard({ template }: { template: TemplateMeta<string> }) {
  return (
